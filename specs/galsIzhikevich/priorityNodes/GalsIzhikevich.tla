...
CanFire(n) ==   /\  state[n].t < MaxTime
                /\  state[n].c[1] = Cardinality(InNeighbours[n])
                /\ \A o \in OutNeighbours[n]: state[n].tDiff[o] < MaxMem - 1

PriorityFire == IF \E n \in PriorityNodes : CanFire(n)
                    THEN \E n \in PriorityNodes : Fire(n)
                    ELSE \E n \in 1..Nodes \ PriorityNodes : Fire(n)

PriorityRecieve ==
        /\  MsgAvailable(messages)
        /\  IF \E m \in GetMsg(messages) : m.out \in PriorityNodes
            THEN \E m \in GetMsg(messages) :
                            /\  m.out \in PriorityNodes
                            /\  Receive(m)
            ELSE \E m \in GetMsg(messages) : Receive(m)


DCGNext ==  \/  PriorityFire
            \/  PriorityRecieve
